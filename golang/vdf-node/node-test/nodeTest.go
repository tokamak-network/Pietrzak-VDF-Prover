...
func NewTestListener(config node.Config) (*TestListener, error) {
...
func (l *TestListener) SubscribeRandomWordsRequested() error {
	var round *big.Int
...
func (l *TestListener) initiateCommitProcess(round *big.Int) {
...
func (l *TestListener) Commit(ctx context.Context, round *big.Int) (common.Address, []byte, error) {
...
func (l *TestListener) GetNextRound() (*big.Int, error) {
...
func (l *TestListener) GetValuesAtRound(ctx context.Context, round *big.Int) (*ValueAtRound, error) {
	config := node.LoadConfig()
	client, err := ethclient.Dial(config.RpcURL)
	if err != nil {
		log.Fatalf("Failed to connect to the Ethereum client: %v", err)
...
func (l *TestListener) AutoRecover(ctx context.Context, round *big.Int, mySender common.Address) error {
	//style := color.New(color.FgHiBlue, color.Bold)
...
func (l *TestListener) FindMinHashAndCompare(ctx context.Context, round *big.Int, mySender common.Address) (bool, common.Address, error) {
...
func (l *TestListener) SubscribeRecovered(ctx context.Context, expectedOmega string, v []BigNumber, x BigNumber, y BigNumber, minSender common.Address) error {
	query := ethereum.FilterQuery{
		Addresses: []common.Address{l.ContractAddress},
		Topics: [][]common.Hash{
			{
...
func (l *TestListener) SubscribeFulfillRandomness(ctx context.Context, round *big.Int, leader common.Address, minSender common.Address) error {
...
func (l *TestListener) Recover(ctx context.Context, round *big.Int, y BigNumber) error {
	fmt.Println("---------------------------------------------------------------------------")
...
func (l *TestListener) handleDisputePhase(round *big.Int) {
	go func() {
		ctx, cancel := context.WithTimeout(context.Background(), ContextTimeout*time.Second)
		defer cancel()
...
func (l *TestListener) proceedWithFulfillRandomness(round *big.Int) {
	ctx, cancel := context.WithTimeout(context.Background(), ContextTimeout*time.Second)
	defer cancel()
...
func (l *TestListener) DisputeLeadershipAtRound(ctx context.Context, round *big.Int) error {
	chainID, err := l.Client.NetworkID(ctx)
	if err != nil {
		return fmt.Errorf("failed to fetch network ID: %v", err)
	}
...
func (l *TestListener) DisputeRecover(ctx context.Context, round *big.Int, v []BigNumber, x BigNumber, y BigNumber) (*types.Transaction, error) {
	chainID, err := l.Client.NetworkID(ctx)
	if err != nil {
...
func (l *TestListener) FulfillRandomness(ctx context.Context, round *big.Int) (*types.Transaction, error) {
...
func (l *TestListener) GetDisputeEndTimeAndLeaderAtRound(ctx context.Context, round *big.Int) (uint64, common.Address, error) {
	config := node.LoadConfig()
	client, err := ethclient.Dial(config.RpcURL)
	if err != nil {
		log.Fatalf("Failed to connect to the Ethereum client: %v", err)
